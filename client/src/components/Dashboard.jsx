...
export default function ProductDetail() {
  ...
  async function handleShipment(e) {
    e.preventDefault()
    setSubmitting(true)
    try {
  ...
  if (loading) return (
    <div style={{ color: 'white', padding: '40px', background: '#0a0a0a', minHeight: '100vh' }}>
      Loading...
    </div>
  )
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
          ...
          <div>
            <div style={{ background: '#111', border: '1px solid #222', borderRadius: '16px', padding: '28px', marginBottom: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '48px', fontWeight: 800, color: getTrustColor(product.trustScore) }}>
                    {product.trustScore}
                  </div>
            ...
            {lastResult && (
              <div style={{ background: lastResult.aiFlag ? '#2a0808' : '#0a2a15', border: `1px solid ${lastResult.aiFlag ? '#ff4444' : '#00ff88'}`, borderRadius: '12px', padding: '16px', marginBottom: '24px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, color: lastResult.aiFlag ? '#ff4444' : '#00ff88' }}>
                      {lastResult.message}
                    </div>
                    <div style={{ color: '#888', fontSize: '13px', marginTop: '2px' }}>
                      AI Risk Score: {lastResult.aiRiskScore}/100 | New Trust Score: {lastResult.trustScore}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              {shipments.length === 0 ? (
                <div style={{ background: '#111', borderRadius: '12px', padding: '32px', textAlign: 'center', color: '#666' }}>
                  No checkpoints yet
                </div>
              ) : (
                <div>
                    ...
                    <div key={s._id} style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        {i < shipments.length - 1 && (
                          <div style={{ width: '2px', flex: 1, background: '#222', marginTop: '4px' }} />
                        )}
                      </div>
          ...
          <div>
            <div style={{ background: '#111', border: '1px solid #222', borderRadius: '16px', padding: '28px', textAlign: 'center', position: 'sticky', top: '20px' }}>
              ...
              <p style={{ color: '#666', fontSize: '12px', marginBottom: '16px' }}>
                Scan to view full product history and Trust Score
              </p>
